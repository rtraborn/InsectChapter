...
\author[1]{R. Taylor Raborn\thanks{Correspondence: rtraborn@indiana.edu}}
...
In insects, TSS profiling has been used to characterize the promoter architecture of \textit{Drosophila melanogaster} \cite{Hoskins:2011io} and subsequently was employed to reveal widespread transposon-driven alternative promoter usage in the fruit fly \cite{Batut:2012kc}. 

In this chapter we discuss the computational analysis of the experimental data derived from of one TSS profiling method, RAMPAGE (RNA Annotation and Mapping of Promoters for Analysis of Gene Expression), that can be used for the precise, quantitative identification of promoters in insect genomes.
We demonstrate this using the software tools GoRAMPAGE \cite{Brendel:2016ab} and TSRchitect \cite{Raborn:2017ab}, providing detailed instructions with the aim of taking the user from raw reads to processed results. 
...
A notable recent methodological advance in TSS Profiling is RAMPAGE \cite{Batut:2012kc,Batut:2013fu}, a protocol for 5$^\prime$-cDNA sequencing that combines cap trapping and template-switching with paired-end sequence information (see Figure 1).
...
There are other TSS profiling methodologies provide paired-end information, although these methods differ (with each other and with RAMPAGE) in the ways capped RNA is captured and processed into finished libraries. 
These include PEAT (Paired-end analysis of transcription) \cite{Ni:2010jh} and nanoCAGE \cite{Plessy:2010kb,Cumbie:2015fpa}. 
PEAT has been applied in two species to date: \textit{D. melanogaster} \cite{Ni:2010jh} and the model plant \textit{Arabidopsis thaliana} \cite{Morton:2014io}, whereas nanoCAGE has been applied to mammalian systems.
While this chapter will discuss the processing and analysis of RAMPAGE libraries, the code and tools we present here are capable of handling any other TSS profiling read datasets.
\\
\\
...
RAMPAGE is currently being applied as part of the latest iteration of ENCODE \cite{Dunham:2012gc} to identify promoters in diverse human tissues \cite{ENCODE:2017bc}, but as of this writing it has not been applied to any non-\textit{Drosophila} insect model system. \\
\\
...
The former is a well-known pay-per-use solution, while the latter two are NSF-funded resources that make compute allocations freely available to the public.\\
For many users, the cyberinfrastructure approach is a convenient solution, particularly when providers offer task-dedicated virtual machines. In that case, the user essentially rents a fully equipped computer with all necessary software pre-installed and sufficient resources for the intended job. For the computational workflows discussed here, researchers can check out an instance of the "bgRAMOSE" image at JetStream which comes with all Brendel Group software (\url{https://brendelgroup.github.io/}) as well as other useful bioinformatics tools enabled.
...
Please find the repository at here (in the folder "demo/MMB/"):\\
\url{https://github.com/brendelgroup/GoRAMPAGE} (\textit{see} \textbf{Note 2}).
...
\noindent
The inputs in this case are BAM files; \textit{TSRchitect} also accepts input in BED format.

\noindent
\begin{verbatim}
DmRAMPAGE <- loadTSSobj(experimentTitle = "RAMPAGE Tutorial", \
 inputDir=BAMDIR, isPairedBAM=TRUE, \
 sampleNames=c("E1h","E2h", "E3h", "L1", "L2", "L3"), \
 replicateIDs=c(1,1,1,2,2,2))
\end{verbatim}

\noindent
Next, we extract TSSs from the alignment files:
...
\item The rRNA sequences were retrieved separately from Genbank at NCBI \cite{Sayers:2012iv}.
...
\item To do this, please edit the batch script \texttt{TSRchitect\_serial\_MMB.R} with a text editor of your choice.
...
The authors are grateful to Thomas W. McCarthy for his help testing the code and providing editorial feedback.
